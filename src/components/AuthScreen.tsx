...
export default function AuthScreen({ onAuthenticated, onBack }: AuthScreenProps) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (!rateLimit.allowed) {
      setError(`LIMITE ATTEINTE - Réessayez dans ${rateLimit.resetIn}s`);
      setLoading(false);
    ...
    if (signInError) {
      ...
      if (attempts + 1 >= MAX_ATTEMPTS) {
        setError('TENTATIVES MAXIMALES DÉPASSÉES - ACCÈS REFUSÉ');
      } else {
          ...
          <div className="terminal-box border-red-500 bg-red-950/20">
            <div className="text-center">
              ...
              <div className="text-red-400 text-sm">
                {'>'} Nombre maximum de tentatives dépassé<br />
                {'>'} Accès au système restreint<br />
    ...
    <div className="min-h-screen bg-black text-zinc-200 font-mono flex items-center justify-center">
      ...
      <div className="max-w-md w-full mx-4">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-xs text-zinc-500">PORTAIL D'ACCÈS ADMIN</p>
        </div>

        <div className="terminal-box">
          <div className="flex items-center space-x-3 mb-6 pb-6 border-b border-zinc-800">
            <Lock className="w-8 h-8 text-amber-600" />
            <span className="text-white font-semibold">[AUTHENTIFICATION_REQUISE]</span>
          </div>

          <form onSubmit={handleLogin} className="space-y-6">
            <div>
              <label className="data-label block mb-2">[IDENTIFIANT]</label>
            ...
            <div>
              <label className="data-label block mb-2">[MOT_DE_PASSE]</label>
              <div className="relative">
